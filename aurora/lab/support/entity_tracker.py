...
class EntityTracker:
    ...
    def _compile_patterns(self) -> None:
        ...
        # 邮箱模式
        self._email_pattern = re.compile(r"[\w.-]+@[\w.-]+\.\w+", re.IGNORECASE)

        # 地址/位置模式
        self._location_patterns = [
            re.compile(
                r"(?:live|moved?|work|located|reside|stay)\s+(?:in|to|at|near)\s+([A-Z][\w\s]+)",
                re.IGNORECASE,
            ),
            re.compile(
                r"(?:address|location|city|place)\s+(?:is|:)?\s*([A-Z][\w\s]+)", re.IGNORECASE
            ),
        ]
        ...
        self._time_patterns = [
            ...
            # "5K time: 25:50"
            re.compile(
                r"(?:5k|5-k|5\s*k)[\s:]+(?:time|pace|result)[\s:]*(\d{1,2}:\d{2}|\d+(?:\.\d+)?\s*min)",
                re.IGNORECASE,
            ),
        ]
    ...
    def extract_entities(self, text: str) -> List[Tuple[str, str, str, float, str]]:
        ...
        running_context = any(
            word in text_lower
            for word in ["run", "race", "5k", "marathon", "pace", "time", "personal", "best", "pb"]
        )
    ...
    def find_potential_updates(
        self, text: str, timestamp: float, similarity_threshold: float = 0.3
    ...
    def check_entity_update(
        self, text: str, plot_id: str, timestamp: float, candidate_plot_id: Optional[str] = None
