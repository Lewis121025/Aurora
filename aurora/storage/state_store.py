...
Provides hot/cold data storage for algorithm state:
- Hot data: Active Stories, Recent Plots -> Redis (fast reads)
- Cold data: Archived Plots, Old Themes -> PostgreSQL JSONB

Design goals:
- Replace pickle-based snapshots with structured JSON serialization
- Support state dict serialization for algorithm classes
- Enable horizontal scaling with shared state stores
"""
...
class StateStore(ABC):
    ...
    Provides hot/cold tier storage:
    - Hot: Low-latency reads (Redis/memory)
    - Cold: Durable, queryable storage (PostgreSQL/SQLite)
    """
    
    @abstractmethod
    async def save_hot(self, user_id: str, key: str, value: Dict[str, Any]) -> None:
        """Save state to hot storage (fast, possibly ephemeral)."""
        ...
    
    @abstractmethod
    async def load_hot(self, user_id: str, key: str) -> Optional[Dict[str, Any]]:
        """Load state from hot storage."""
        ...
    
    @abstractmethod
    async def delete_hot(self, user_id: str, key: str) -> bool:
        """Delete from hot storage. Returns True if existed."""
        ...
    
    @abstractmethod
    async def save_cold(self, user_id: str, key: str, value: Dict[str, Any]) -> None:
        """Save state to cold storage (durable)."""
        ...
    ...
    async def load_latest_snapshot(self, user_id: str) -> Optional[Tuple[int, Dict[str, Any]]]:
        """Load the latest snapshot. Returns (seq, state_dict) or None."""
...
class RedisPostgresStateStore(StateStore):
    """Production state store with Redis (hot) + PostgreSQL (cold).
    ...
    async def _ensure_postgres(self):
        """Lazily create PostgreSQL connection pool."""
    ...
    async def list_keys(self, user_id: str, prefix: str = "", tier: str = "hot") -> List[str]:
        if tier == "hot":
            ...
            # Strip the prefix to return just the key portion
...
class SQLiteStateStore(StateStore):
    """SQLite-based state store for single-node deployments.
    
    Uses a single SQLite database with separate tables for hot/cold tiers.
    """
    ...
    async def _ensure_conn(self):
        """Lazily create SQLite connection."""
...
def create_state_store(
    ...
    **kwargs,
    ...
    Args:
        backend: "memory", "sqlite", or "redis_postgres"
