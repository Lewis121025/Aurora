...
class AbstentionDetector:
    ...
    def detect(
        ...
        existence_query_patterns = [
            "did we ever", "have we ever", "did i ever", "have i ever",
            "was there ever", "is there any mention", "any information about",
            "do you know if", "did you mention", "have you mentioned",
            "我们有没有", "有没有提到", "有没有讨论", "是否提到过",
            "是否讨论过", "有记录吗", "提到过吗"
        ]
        ...
        if len(retrieved_scores) >= 3:
            ...
            if score_std < self.uniform_score_std_threshold and top_score < self.low_score_threshold:
                return AbstentionResult(
                    should_abstain=True,
                    confidence=0.7,
                    reason=(
