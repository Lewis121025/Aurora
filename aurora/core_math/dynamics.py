...
def prediction_error(
    cue: np.ndarray,
    latent: np.ndarray,
    vad_cue: list[float] | None = None,
    vad_latent: list[float] | None = None,
) -> float:
    ...
    if vad_cue and vad_latent and len(vad_cue) == 3 and len(vad_latent) == 3:
        ...
        vad_err = (
            sum((cue_v - latent_v) ** 2 for cue_v, latent_v in zip(vad_cue, vad_latent)) / 12.0
        )
...
def update_metric(
    metric: MetricState, cue: np.ndarray, error: float, lr: float = 0.05
) -> MetricState:
