    ...
    def extract(self, text: str) -> List[ExtractedFact]:
        ...
        for pattern in self._compiled_patterns.get("action", []):
            for match in pattern.finditer(text_lower):
                ...
                if len(fact_text) >= self.min_fact_length:
                    facts.append(
                        ExtractedFact(fact_text=fact_text, fact_type="action", entities=[action])
                    )
        ...
        for pattern in self._compiled_patterns.get("location", []):
            for match in pattern.finditer(text_lower):
                ...
                if len(fact_text) >= self.min_fact_length:
                    facts.append(
                        ExtractedFact(
                            fact_text=fact_text, fact_type="location", entities=location_parts
                        )
                    )
        ...
        for pattern in self._compiled_patterns.get("time", []):
            for match in pattern.finditer(text_lower):
                ...
                if len(fact_text) >= self.min_fact_length:
                    facts.append(
                        ExtractedFact(fact_text=fact_text, fact_type="time", entities=[time_expr])
                    )
