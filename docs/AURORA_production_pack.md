...
- 关键入口：
  - CLI demo：`python -m aurora.cli demo`
  - API（可选）：`uvicorn aurora.api.app:app --port 8000`
...
- `aurora/service.py`：**AuroraTenant**
...
在 `aurora/algorithms/aurora_core.py` 我做了**最小侵入式补丁**：
...
- `aurora/llm/schemas.py`：`PlotExtraction / StoryUpdate / ThemeCandidate / SelfNarrativeUpdate / ContradictionJudgement`
- `aurora/llm/prompts.py`：全部 prompt 模板（带 “ONLY JSON” 指令）
...
- `aurora/service.py`：单租户生产封装（幂等/回放/快照）
- `aurora/hub.py`：多租户 LRU
- `aurora/storage/event_log.py`：sqlite append-only event log
- `aurora/storage/snapshot.py`：pickle snapshot
- `aurora/storage/doc_store.py`：sqlite derived doc store
- `aurora/llm/*`：schema + prompts + mock provider
- `aurora/algorithms/aurora_core.py`：核心算法（已补确定性 id）
- `aurora/api/app.py`：FastAPI endpoints（可选）
- `tests/*`：确定性回放/幂等测试
