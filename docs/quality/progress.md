...
#### 2. 质量检查工具
- [x] 创建统一质量检查脚本（`scripts/quality/check.py`）
...
- [x] 完成仓库内所有导入迁移
- [x] 删除 `constants.py`
- [x] 创建自动迁移脚本（`aurora/scripts/migrations/constants.py`）
...
# 严格模式（CI）
python scripts/quality/check.py --strict
```
...
```bash
# 预览迁移
python -m aurora.scripts.migrations.constants aurora/ --dry-run
