...
from aurora.lab.causal.graph import CausalMemoryGraph
...
class TestCausalEdgeBelief:
    ...
    def test_effective_causal_weight(self):
        belief = CausalEdgeBelief(
            edge_type="causal",
            dir_a=9.0,
            dir_b=1.0,  # Strong forward direction
            str_a=8.0,
            str_b=2.0,  # Strong causal strength
            conf_a=1.0,
            conf_b=9.0,  # Low confounding
        )
...
class TestInterventionEngine:
    ...
    def test_do_intervention_basic(self, metric, sample_plots):
        ...
        # Add causal beliefs
        beliefs = {
            ("plot_a", "plot_b"): CausalEdgeBelief(
                edge_type="causal", dir_a=9.0, dir_b=1.0, str_a=8.0, str_b=2.0
            ),
            ("plot_b", "plot_c"): CausalEdgeBelief(
                edge_type="causal", dir_a=9.0, dir_b=1.0, str_a=8.0, str_b=2.0
            ),
        }
...
class TestCounterfactualReasoner:
    ...
    def test_counterfactual_query(self, metric, sample_plots):
        ...
        factual_values = {p.id: p.embedding for p in sample_plots}
